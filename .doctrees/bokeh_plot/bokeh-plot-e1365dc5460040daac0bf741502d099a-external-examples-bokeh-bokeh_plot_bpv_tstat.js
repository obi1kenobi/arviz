(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("ec643574-07ab-45ef-9cad-06b78dfe4413");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'ec643574-07ab-45ef-9cad-06b78dfe4413' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.2.1.min.js": "qkRvDQVAIfzsJo40iRBbxt6sttt0hv4lh74DG7OK4MCHv4C5oohXYoHUM5W11uqS", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.1.min.js": "Sb7Mr06a9TNlet/GEBeKaf5xH3eb6AlCzwjtU82wNPyDrnfoiVl26qnvlKjmcAd+", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.1.min.js": "HaJ15vgfmcfRtB4c4YBOI4f1MUujukqInOWVqZJZZGK7Q+ivud0OKGSTn/Vm2iso"};
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.2.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.1.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"6d99de6a-183f-41b2-a6a9-095fb89a67f9":{"roots":{"references":[{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"65348","type":"Line"},{"attributes":{},"id":"65328","type":"ResetTool"},{"attributes":{},"id":"65334","type":"SaveTool"},{"attributes":{},"id":"65316","type":"LinearScale"},{"attributes":{"data":{},"selected":{"id":"65365"},"selection_policy":{"id":"65366"}},"id":"65352","type":"ColumnDataSource"},{"attributes":{"text":"y","text_font_size":{"value":"15pt"}},"id":"65376","type":"Title"},{"attributes":{"children":[[{"id":"65311"},0,0]]},"id":"65392","type":"GridBox"},{"attributes":{"axis":{"id":"65324"},"dimension":1,"ticker":null},"id":"65327","type":"Grid"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"65372","type":"Circle"},{"attributes":{"below":[{"id":"65320"}],"center":[{"id":"65323"},{"id":"65327"},{"id":"65369"}],"left":[{"id":"65324"}],"output_backend":"webgl","plot_height":500,"plot_width":500,"renderers":[{"id":"65350"},{"id":"65355"},{"id":"65374"}],"title":{"id":"65376"},"toolbar":{"id":"65338"},"toolbar_location":null,"x_range":{"id":"65312"},"x_scale":{"id":"65316"},"y_range":{"id":"65314"},"y_scale":{"id":"65318"}},"id":"65311","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"65387","type":"Selection"},{"attributes":{"overlay":{"id":"65337"}},"id":"65332","type":"LassoSelectTool"},{"attributes":{},"id":"65388","type":"UnionRenderers"},{"attributes":{},"id":"65366","type":"UnionRenderers"},{"attributes":{"formatter":{"id":"65360"},"ticker":{"id":"65321"}},"id":"65320","type":"LinearAxis"},{"attributes":{},"id":"65325","type":"BasicTicker"},{"attributes":{},"id":"65364","type":"UnionRenderers"},{"attributes":{},"id":"65362","type":"BasicTickFormatter"},{"attributes":{"axis":{"id":"65320"},"ticker":null},"id":"65323","type":"Grid"},{"attributes":{"toolbar":{"id":"65393"},"toolbar_location":"above"},"id":"65394","type":"ToolbarBox"},{"attributes":{"toolbars":[{"id":"65338"}],"tools":[{"id":"65328"},{"id":"65329"},{"id":"65330"},{"id":"65331"},{"id":"65332"},{"id":"65333"},{"id":"65334"},{"id":"65335"}]},"id":"65393","type":"ProxyToolbar"},{"attributes":{},"id":"65331","type":"WheelZoomTool"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"65353","type":"Line"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"xs_units":"screen","ys_units":"screen"},"id":"65337","type":"PolyAnnotation"},{"attributes":{"data_source":{"id":"65347"},"glyph":{"id":"65348"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65349"},"selection_glyph":null,"view":{"id":"65351"}},"id":"65350","type":"GlyphRenderer"},{"attributes":{"data":{},"selected":{"id":"65387"},"selection_policy":{"id":"65388"}},"id":"65371","type":"ColumnDataSource"},{"attributes":{},"id":"65363","type":"Selection"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"65363"},"selection_policy":{"id":"65364"}},"id":"65347","type":"ColumnDataSource"},{"attributes":{"callback":null},"id":"65335","type":"HoverTool"},{"attributes":{},"id":"65314","type":"DataRange1d"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"65349","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"65354","type":"Line"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"65355"}]},"id":"65370","type":"LegendItem"},{"attributes":{},"id":"65360","type":"BasicTickFormatter"},{"attributes":{},"id":"65321","type":"BasicTicker"},{"attributes":{"source":{"id":"65352"}},"id":"65356","type":"CDSView"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"65328"},{"id":"65329"},{"id":"65330"},{"id":"65331"},{"id":"65332"},{"id":"65333"},{"id":"65334"},{"id":"65335"}]},"id":"65338","type":"Toolbar"},{"attributes":{},"id":"65318","type":"LinearScale"},{"attributes":{"data_source":{"id":"65352"},"glyph":{"id":"65353"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65354"},"selection_glyph":null,"view":{"id":"65356"}},"id":"65355","type":"GlyphRenderer"},{"attributes":{"overlay":{"id":"65336"}},"id":"65330","type":"BoxZoomTool"},{"attributes":{},"id":"65365","type":"Selection"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","top_units":"screen"},"id":"65336","type":"BoxAnnotation"},{"attributes":{"source":{"id":"65371"}},"id":"65375","type":"CDSView"},{"attributes":{"children":[{"id":"65394"},{"id":"65392"}]},"id":"65395","type":"Column"},{"attributes":{"formatter":{"id":"65362"},"ticker":{"id":"65325"}},"id":"65324","type":"LinearAxis"},{"attributes":{"items":[{"id":"65370"}]},"id":"65369","type":"Legend"},{"attributes":{"data_source":{"id":"65371"},"glyph":{"id":"65372"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65373"},"selection_glyph":null,"view":{"id":"65375"}},"id":"65374","type":"GlyphRenderer"},{"attributes":{},"id":"65329","type":"PanTool"},{"attributes":{},"id":"65312","type":"DataRange1d"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"65373","type":"Circle"},{"attributes":{"source":{"id":"65347"}},"id":"65351","type":"CDSView"},{"attributes":{},"id":"65333","type":"UndoTool"}],"root_ids":["65395"]},"title":"Bokeh Application","version":"2.2.1"}}';
                  var render_items = [{"docid":"6d99de6a-183f-41b2-a6a9-095fb89a67f9","root_ids":["65395"],"roots":{"65395":"ec643574-07ab-45ef-9cad-06b78dfe4413"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();